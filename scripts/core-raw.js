...
function session_ttl()
{
    jQuery.ajax(
	{
		url: "shiponline/curl.php?action=session_ttl", type: "POST",
		async: false,
...
{   
    ...
    jQuery.ajax({
        url: "shiponline/curl.php?action=" + action,
        type: 'POST',
        cache: false,
        dataType: 'json',
        data: post,
...
{   
    ...
    if (nxt >= total)
    {
        ...
        {
			delete window.goods_set_divison;
    ...
    $(window).scrollTop($('#shipon_accordion').offset().top);
}
...
{
	var data = jQuery('#shipon_account_settings').serialize();
...
function shipon_goods_remove(reference)
{
	...
	$("#shipon_goods_table tbody tr").removeAttr('class');
	shipon_goods_fix_attrs();
...
function setup_autocomplete()
{
    jQuery(".shipon_autocomplete").autocomplete(
	{
	    source: function (request, response)
	    {
	        ...
	        jQuery.ajax(
			{
			    url: "shiponline/curl.php?action=autocomplete", type: "POST", dataType: "json",
			    data: { 
					term: request.term, 
					type: 'source',
					dest: dest,
...
function appointment_toggle(reference)
{
    ...
    else
    {
        inputs.each(function ()
        {
            if (!jQuery(this).attr('disabled') && jQuery(this).attr('type') != 'checkbox') jQuery(this).attr('disabled', true); 
        });
...
function print_pdf()
{
    ...
    jQuery.ajax(
		{
			url: "curl.php?action=get_printer_friendly_bill", type: "POST",
			data: {
			    bill: bol
			},
			success: function (data)
...
{
	$('#shipon_tracking_search').attr('value',query);
    update_tracking();
}
...
function fetch_goods_select(division)
{
    ...
    jQuery.ajax(
		{
			url: "shiponline/curl.php?action=view", 
			type: "POST",
            async: false,
			data: {
				division: division,
